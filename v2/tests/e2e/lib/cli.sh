...
# path resolution: v2 builds clawcli into v2/target/release/clawcli
# (workspace lives at v2/Cargo.toml).

# Resolve clawcli binary. Prefer release build in v2/target/, fall back
...
e2e_cli_bin() {
    ...
    if [ -x "$repo_root/v2/target/release/clawcli" ]; then
        echo "$repo_root/v2/target/release/clawcli"
    elif [ -x "$repo_root/v2/target/debug/clawcli" ]; then
        echo "$repo_root/v2/target/debug/clawcli"
    else
