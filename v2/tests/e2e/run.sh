...
#   ./v2/tests/e2e/run.sh smoke-mac-sandbox-noproxy
#   ./v2/tests/e2e/run.sh --skip-proxy all
#   ./v2/tests/e2e/run.sh --keep-home <scenario>     # leave HOME on disk
#
# Adapted from v1 tests/e2e/run.sh. v2 differences:
# - Repo root resolves to .../ClawEnv (the parent of v2/), so the lib
#   helpers can find target/release/clawcli under v2/target/.
...
# Repo root = three parents up from this script (ClawEnv/v2/tests/e2e/).
...
if [ "${#SELECTION[@]}" -eq 0 ]; then
    ...
    echo "Scenarios:" >&2
    ls "$E2E_REPO_ROOT/v2/tests/e2e/scenarios/" | sed 's/\.sh$//' | sed 's/^/  /' >&2
    exit 2
fi
...
if [ "${SELECTION[0]}" = "all" ]; then
    SELECTION=()
    for f in "$E2E_REPO_ROOT/v2/tests/e2e/scenarios/"*.sh; do
        [ -f "$f" ] || continue
        name=$(basename "$f" .sh)
...
# Source libraries
# ──────────────────────────────────────────────────────────────
source "$E2E_REPO_ROOT/v2/tests/e2e/lib/isolate.sh"
source "$E2E_REPO_ROOT/v2/tests/e2e/lib/cli.sh"
source "$E2E_REPO_ROOT/v2/tests/e2e/lib/assert.sh"
source "$E2E_REPO_ROOT/v2/tests/e2e/lib/detect-proxy.sh"
source "$E2E_REPO_ROOT/v2/tests/e2e/lib/prewarm.sh"
source "$E2E_REPO_ROOT/v2/tests/e2e/lib/preflight.sh"
...
for i in "${!SELECTION[@]}"; do
    name="${SELECTION[$i]}"
    script="$E2E_REPO_ROOT/v2/tests/e2e/scenarios/${name}.sh"
